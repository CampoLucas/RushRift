...
public class GhostPlayer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float dur = Duration(AsFrames(loadedRun), loadedRun.durationSeconds);
        if (dur <= 0f) return;
    ...
    public void LoadBestGhost()
    {
        ...
        {
            loadedRun = null; cachedPositions.Clear(); debugLoadedGhostPath = "";
    ...
    private bool HasValidRun() =>
        loadedRun != null && loadedRun.frames != null && loadedRun.frames.Count >= 2 && loadedRun.durationSeconds > 0f;

    private void ApplyPose(float t, float dt)
    {
        var frames = AsFrames(loadedRun);
    ...
    private static List<Frame> AsFrames(GhostRecorder.GhostRunData run)
    {
        var list = new List<Frame>(run.frames.Count);
        for (int i = 0; i < run.frames.Count; i++)
        {
            var f = run.frames[i];
            list.Add(new Frame { time = f.time, position = f.position, rotation = f.rotation });
        }
        return list;
    }
